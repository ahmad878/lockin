...
app.post('/register-fcm-token', async function(req, res) {
  ...
  // Serve chat page (HTML) for a specific chat ID
  app.get('/chat/:chatId', async function(req, res, next) {
    // Skip if it looks like an API call (has specific patterns)
    if (req.params.chatId === 'id' || req.params.chatId.includes('@')) {
      return next();
    }
    
    // Serve the chat.html file - the page will verify access via API
    const chatPath = path.join(__dirname, 'public', 'chat.html');
    if (require('fs').existsSync(chatPath)) {
      res.sendFile(chatPath);
    } else {
      res.status(404).json({ success: false, message: 'Chat page not found' });
    }
  });

  // Get chat by ID (API) - verifies user is part of chat
  app.get('/chat/id/:chatId', async function(req, res) {
    try {
      const token = req.cookies[COOKIE_NAME];
      if (!token) {
        return res.status(401).json({
      ...
      // Verify user is part of this chat (either person1 or person2)
      const userId = decoded.id;
      const isPerson1 = chat.person1.userId === userId;
      const isPerson2 = chat.person2.userId === userId;
      ...
      const otherUser = isPerson1 ? {
        id: chat.person2.userId,
        email: chat.person2.email,
        name: chat.person2.name
      } : {
        id: chat.person1.userId,
        email: chat.person1.email,
        name: chat.person1.name
      };
  ...
  // Get or create chat with a contact (returns chat ID)
  app.post('/chat/start', async function(req, res) {
    try {
      const token = req.cookies[COOKIE_NAME];
      if (!token) {
        return res.status(401).json({
      ...
      const { contactEmail, contactUserId, contactName } = req.body;
      
      if (!contactEmail && !contactUserId) {
        return res.status(400).json({
          success: false,
          message: 'Contact email or userId required'
        });
      }
      ...
      if (contactUserId) {
        ...
        contactUser = await User.findOne({ email: contactEmail.trim().toLowerCase() });
      }
      ...
      const newChat = new Chat({
        ...
        person2: {
          userId: contactUser._id.toString(),
          email: contactUser.email,
          name: contactName || contactUser.fullName
        },
        messages: [],
        lastMessage: {
          message: '',
  ...
  // Get chat messages with a specific contact (legacy - by email)
  app.get('/chat/email/:contactEmail', async function(req, res) {
    try {
      const token = req.cookies[COOKIE_NAME];
      if (!token) {
        return res.status(401).json({
